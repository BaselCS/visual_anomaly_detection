...
DEFAULT_EVAL_CONFIG = {
    ...
    "calibration_fraction": 0.2,                # Fraction of data used for calibration vs evaluation
    "reconstruction_strength": 0.38,  # Strength for img2img reconstruction (0.0 = perfect copy, 1.0 = full generation)
    "reconstruction_guidance_scale": 7.5,  # Guidance scale for reconstruction (higher = more faithful to prompt, but may reduce diversity)
...
def calculate_anomaly_score(original_image, reconstructed_image):
    ...
    combined_score = (
        ...
        0.40 * lpips_distance + 
        0.20 * max_patch_score  # التركيز على أعلى نقطة شذوذ
    )
